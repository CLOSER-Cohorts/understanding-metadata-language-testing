...
!!! example ""
    ...
    **Intended learning outcome**
    ...
    - Explain the benefits of creating and using metadata
    - Explain the role of metadata in implementing the FAIR principles
    - Identify stakeholders who create and use metadata

## Introduction to metadata

In the last [unit,](<1.3 FAIR.md>) we identified metadata as a fundamental component of implementing the FAIR principles. In this unit, we'll examine what metadata is.
...
!!! abstract ""
    ...
    **Let's begin with an analogy.**
    ...
    Imagine it's a hot day and you're thirsty. You have the option of two drinks.<sup>[1]</sup>
...
As Option 1 does not have a label, we don't know ...
...
!!! question "How does the drink analogy link to metadata in research?"

    The drink inside the can represents a _dataset_ from a research study. The label providing the information about the drink represents the _metadata_. 
	
	Metadata describes the dataset, telling us how the dataset was produced and what it contains. Like the unlabelled can, if a research study doesn't have metadata, we don't know any contextual information such as _who_ collected the data, _when_ was the data collected or _where._ We also don't know what data exists within the datasets.

    Similar to storing the can in a supermarket, data repositories and data catalogues store metadata for different research projects in the same place, allowing us to search and compare across different projects. This increases the trustworthiness of the project and its data. We will cover data repositories and data catalogues in more detail in [unit 2.3](<2.3 Using metadata - discover data.md>)
...
!!! quote "According to CODATA, [metadata](https://terms.codata.org/rdmt/metadata) is ..."
	...
	_'Data about data. It is data (or information) that defines and describes the characteristics of other data. It is used to improve the understanding and use of the data.' <sup>[4]</sup>_

!!! quote "The National Information Standards Organization offers a similar definition ..."

	_'Metadata is structured information that describes, explains, locates, or otherwise makes it easier to retrieve, use, or manage an information resource. Metadata is often called data about data or information about information.' <sup>[5]</sup>_
...
!!! tip ""
    1. **Findability**: Metadata helps us find resources and makes our resources (e.g. datasets) findable.
    2. **Understandability and context**: Metadata helps us to quickly understand resources for example, what the resources consist of and how they were produced.
    3. **Use and action**: Metadata provides us with the information on how to access and use the resource including any limitations around this.
...
!!! abstract ""
    ...
    When identifying relevant secondary data for your project, you come across this dataset below <sup>[6]</sup>. You need to determine whether it would be useful for your research.
...
??? question "Expand the box to explore questions about the dataset"
...
Select each tab to see how we group the questions.
...
Metadata can describe the overall study, giving an overview of the whole research effort. Study level metadata is particularly useful when we're going through a large amount of research. It also gives us supporting information around the dataset that may not be included in the dataset itself, for example who funded the research and who to contact for more information about the project.
...
!!! abstract ""
	...
	You can find study level metadata for the unlabelled dataset [on CLOSER Discovery](https://discovery.closer.ac.uk/Item/uk.cls.ncds/524c8f92-493f-4660-90c4-0aa7ac3d2640/)<sup>[7]</sup>. CLOSER Discovery is a metadata catalogue that pulls together metadata from different UK longitudinal population studies in one place, so people can search, discover and understand research (we will cover data and metadata catalogues in more detail in [unit 2.3](<2.3 Using metadata - discover data.md>). 

    Using the study metadata available on [CLOSER Discovery,](https://discovery.closer.ac.uk/Item/uk.cls.ncds/524c8f92-493f-4660-90c4-0aa7ac3d2640/) try answering some of the questions above.

    We know the study was conducted across **England, Scotland and Wales** and produced as part of the **National Child Development Study**.<sup>[8]</sup>
...
!!! abstract ""
	...
	You can explore metadata for the unlabelled dataset [on CLOSER Discovery here](https://discovery.closer.ac.uk/item/uk.cls.ncds/a52ee654-efa8-4839-a545-fd82bec63e71)<sup>[9]</sup>. Using this metadata, try answering some of the questions above.
...
While dataset metadata provides more information about how and when the data is collected, we still don't know what the column headers mean or what the numbers represent so we can't interpret the data. Variable metadata is a key tool that enables us to do this. 

We use the term variable all the time in research, but what is its definition?

!!! quote "CODATA defines a data variable as..."
    _In data collection, refers to a characteristic, number, property or quantity that can be observed, measured or recorded. May alter in state and/or value._ <sup>[10]</sup>

!!! quote "The metadata standard, DDI Alliance, defines a variable as..."
    _'... a characteristic of a unit being observed. A variable might be the answer of a question, have an administrative source, or be derived from other variables._ <sup>[11]</sup>
...
Metadata can provide information about the variables to help us understand the data further. For example...
...
!!! abstract ""
	...
	Information about the data can be found [on CLOSER Discovery here](https://discovery.closer.ac.uk/item/uk.cls.ncds/b3909a8c-2118-4f82-ad34-245ce610ea07) under the topic tab.<sup>[12]</sup> You can select different variables to gain insight into what data they collect and how the data was collected (e.g. the question asked to the participant in the study).

	For example, we can see that the variable [N2Region](https://discovery.closer.ac.uk/item/uk.cls.ncds/1358207b-5bee-43e4-ba1f-e94895a5bf0f) refers to the region the participant is living in and that '1' = North, '2' = North East, '3' = E & W Riding and '4' = North Midlands. This helps us determine what data within the dataset is relevant to our study as we are only examining the northern regions of England.
...
!!! abstract ""
	...
	Metadata about the survey [can be explored here](https://discovery.closer.ac.uk/item/uk.cls.ncds/b8698e54-0501-46e4-962c-cf6e2906fe8e/)<sup>[13]</sup>.

## Challenges of not having metadata

!!! question "If you didn't have metadata, how would you go about finding the information you need to understand the data?"
...
Consider these two descriptions of a dataset<sup>[14]</sup>.
...
- **Description two**  
    Description two is available online on a data catalogue site <sup>[15]</sup> (we will cover data catalogues in more depth in [unit 2.3](<2.3 Using metadata - discover data.md>)).
    <p></p>
...
!!! question "What are the differences between these two descriptions?"
    ...
    ??? info "Differences between the two descriptions"
...
Think about how you write your address in an online form. In which version, do you think a machine will understand what ‘Big Town’ is?

<img src="../img/address_example.png" alt="Alt Text" width="750" height="500">

In example b. each item (element) of the name and address has a separate meaning. 

Metadata works by breaking information into clear, separate elements, so that both humans and machines can understand exactly what each piece of information means.

We can often (but not always) interpret the information without explanation, but we can misinterpret information without clear structure and defintions, for example, the name: Lee Thomas. Is Lee the first name or surname? Could it be Thomas Lee? 

Machines stuggle even more, a computer does not:

- understand context,
- recognise common naming patterns or
- interpret formatting clues, so it cannot reliably determine what part of the address is the city for example.

Humans can often infer meaning. Machines cannot, they rely on structure.

Without structured metadata:

- We cannot automate systems
- Data cannot be reliably searched or filtered
- Datasets cannot be easily combined or compared
...
=== "Machine readable and machine actionable"
	...
	By describing how different metadata elements relate to each other, for example what categories they're in or if a piece of information is linked to another piece of information, machines can not only read the metadata but also conduct further actions. 
	
	For example, we can use computers to search and filter our metadata using sites such as data repositories and data catalogues. We'll examine this more closely at these in [unit 2.3](<2.3 Using metadata - discover data.md>).
...
??? tip "Examples of where we use metadata outside of a research context"
...
- **Policy** passed by national governments that require researchers to make their metadata widely available in order to encourage the discoverability and reusability of research. For example, [Act No 130/2002 Coll. on the Support of Research and Development from Public Funds](https://mzv.gov.cz/jnp/en/foreign_relations/science_and_technology/key_documents_on_czech_research_and.html)<sup>[16]</sup> passed in Czech Republic, that requires all projects using public funds to publish their metadata<sup>[17]</sup>.

Thinking back to [unit 1.2](<1.2 Research data management.md>) [Research Data Management](https://terms.codata.org/rdmt/research-data-management "Storage, access and preservation of data created or collected in the course of research. Research data management practices cover the entire lifecycle of the data, from planning the investigation to conducting it, and from backing up data as it is created and used to long term preservation of data deliverables after the research investigation has concluded.<sup>[18]</sup>.") (RDM), metadata is a core component of RDM and should be included in all Data Management Plans (DMPs).

## Metadata and FAIR

Thinking back to the [previous unit](<1.3 FAIR.md>), metadata is a key part of making data [FAIR](https://terms.codata.org/rdmt/fair-data "Data that is managed in such a way that it is findable, accessible, interoperable, and reusable (FAIR).<sup>[19]</sup>."). When we talk about FAIR data, we usually think about only about the data itself, but the metadata also needs to be FAIR. If your metadata is well-structured and follows FAIR principles, it makes your data easier to find, understand, and reuse, giving you and others the full benefit.

Select the tabs below to explore how metadata makes our data FAIR and what steps we can take to make our data FAIR.
...
The guidelines below explore how to make both our data and metadata FAIR<sup>[19]</sup>...

<img src="../img/FAIR_metadata_infographic.png" alt="Alt Text" width="750" height="500">

In the following units, we explore how we can implement this guidance using data repositories and catalogues [unit 2.3](<2.3 Using metadata - discover data.md>)), controlled vocabularies ([unit 2.4](<2.4 Controlled vocabularies.md>) and metadata standards ([unit 2.5](<2.5 Metadata standards.md>)).
...
!!! question ""
    ...
    **True or false...**
    ...
    - FAIR principles only apply to published datasets, not metadata.
        ...
        ??? success "Reveal answer"
            ...
            **FALSE** FAIR principles apply to both data *and* metadata. Metadata is essential for making data findable, accessible, and reusable.

    - Metadata helps ensure data is interpretable by both humans and machines.

        ??? success "Reveal answer"

            **TRUE** Metadata provides context, structure, and standardisation, allowing both people and machines to understand and use the data correctly.

    - Metadata is only relevant for digital datasets.

        ??? success "Reveal answer"

            **FALSE** Metadata can also describe non-digital data, such as physical samples or paper records, helping document their context and meaning.

    - It is always optional to create metadata.

        ??? success "Reveal answer"

            **FALSE** While not always enforced, creating metadata is a fundamental part of good research practice and is often required by funders, repositories, and FAIR guidelines.
    ...
    **Which of the following is a key purpose of metadata in research?**
    ...
    ??? success "Reveal answer"
        ...
        2. Describing data content and structure. Metadata is used to describe what the data contains, how it is structured, and how it was created. This makes it easier to understand, find, and reuse. The other options relate to data processing or security, not metadata.

    **Which of the following is _not_ typically included in research metadata?**
    ...
    ??? success "Reveal answer"
        ...
        3. Data analysis results. Metadata describes the data, not the findings derived from it. Information such as file size, creator, and licensing are standard metadata elements, whereas analysis results belong to research outputs, not metadata.

    **Which format can you _not_ create metadata? Select all that are relevant.**
    ...
    ??? success "Reveal answer"
        ...
        .txt and Word. Metadata is best created in structured, machine-readable formats such as JSON or XML, which support interoperability and reuse. While you *can* write descriptive information in plain text or Word documents, these formats are not structured or standardised, making them less suitable for formal metadata creation in FAIR data practices. You can also document metadata using CSV if a structured, consistent format is used.


---
## Resources
You can find more 'Metadata Introduction' resources on the [resources page](<../resources.md#metadata-introduction>).
...
- <sup>[2]</sup> Scottish Grocers (2025) 'Irn Bru Regular Cans' [Online image](https://scottishgrocers.co.uk/product/beverages/soft-drinks/irn-bru-regular-cans/)
- <sup>[3]</sup> The Little Market (2025) 'IRN-BRU Original 330 ml' [Online image] (https://thelittlemarket.sg/product/irn-bru-sparking-soft-drink-330m/?srsltid=AfmBOophDjebL659FdEE7LGdvSs6ZnJ9MND2BzNMrvpt7u5PpqsBhgeY)
- <sup>[4]</sup> CODATA (2025) RDM Terminiology: [metadata](https://terms.codata.org/rdmt/metadata)
- <sup>[5]</sup> Riley, J. (2017) ['Understanding metadata: what is metadata, and what is it for?']((http://www.niso.org/publications/press/UnderstandingMetadata.pdf)) _National Information Standards Organization._ Baltimore.
- <sup>[6]</sup> University of London, Institute of Education, Centre for Longitudinal Studies. (2024). _National Child Development Study: Childhood Data from Birth to Age 16, Sweeps 0-3, 1958-1974._ [data collection]. 3rd Edition. National Children's Bureau, National Birthday Trust Fund, [original data producer(s)]. UK Data Service. SN: 5565, DOI: http://doi.org/10.5255/UKDA-SN-5565-2
...
- <sup>[9]</sup> CLOSER (2025) ['Age 11 Survey (1969)' _CLOSER Discovery_](https://discovery.closer.ac.uk/item/uk.cls.ncds/a52ee654-efa8-4839-a545-fd82bec63e71)
- <sup>[10]</sup> CODATA (2025) RDM Terminology Bank: [Variable](https://rdf-vocabulary.ddialliance.org/discovery.html#variable-and-variable-definition)
- <sup>[11]</sup> DDI Alliance (2025) [DDI-RDF Discovery Vocabulary (Disco)](https://rdf-vocabulary.ddialliance.org/discovery.html#variable-and-variable-definition)) World Wide Web Consortium (WC3) 
- <sup>[12]</sup> CLOSER (2025) ['NCDS2 Parental Interview Form (1969) Dataset' _CLOSER Discovery_](https://discovery.closer.ac.uk/item/uk.cls.ncds/a52ee654-efa8-4839-a545-fd82bec63e71)
- <sup>[13]</sup> CLOSER (2025) ['NCDS Age 11 Parental Interview Form' _CLOSER Discovery_](https://discovery.closer.ac.uk/item/uk.cls.ncds/b8698e54-0501-46e4-962c-cf6e2906fe8e/)
- <sup>[14]</sup> University of London, Institute of Education, Centre for Longitudinal Studies. (2024). _National Child Development Study: Childhood Data from Birth to Age 16, Sweeps 0-3, 1958-1974._ [data collection]. 3rd Edition. National Children's Bureau, National Birthday Trust Fund, [original data producer(s)]. UK Data Service. SN: 5565, DOI: http://doi.org/10.5255/UKDA-SN-5565-2
- <sup>[15]</sup> CLOSER (2025) ['Age 11 Survey (1969)' _CLOSER Discovery_](https://discovery.closer.ac.uk/item/uk.cls.ncds/a52ee654-efa8-4839-a545-fd82bec63e71)
- <sup>[16]</sup> Parliament of the Czech Republic (2009) [ACT No. 130/2002 Coll. on the Support of Research and Development from Public Funds and on the Amendment to Some Related Acts](https://vyzkum.gov.cz/FrontClanek.aspx?idsekce=15607) _Czech Republic_
- <sup>[17]</sup> Ministry of Foreign Affairs of the Czech Republic (2020) ['Key Documents on Czech Research and Innovation'](https://mzv.gov.cz/jnp/en/foreign_relations/science_and_technology/key_documents_on_czech_research_and.html)
- <sup>[18]</sup> CODATA (2025) RDM Terminology: [Research data management](https://terms.codata.org/rdmt/research-data-management)
- <sup>[19]</sup> CODATA (2025) RDM Terminology: [FAIR data](https://terms.codata.org/rdmt/fair-data)
- <sup>[20]</sup>[Go FAIR (2025) FAIR Principles](www.go-fair.org/fair-principles/)
- <sup>[21]</sup> UGent University (2021) Open Science Knowledge clip: Metadata [Online video] www.youtube.com/watch?v=DW2T_cnqKPU) 
- <sup>[22]</sup> CESSDA (2025) 'Documentation and Metadata' Data Management Expert Guide www.dmeg.cessda.eu/Data-Management-Expert-Guide/2.-Organise-Document/Documentation-and-metadata
